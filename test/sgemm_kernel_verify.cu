...
__global__ void sgemm_v2(float* A, float* B, float* C, const int m,const int n,const int k, float alpha, float beta){
    ...
    for (int i = 0 ; i < k/BK; i ++){
        s_a[ty][tx] = float(A[iy * k + i * BK + tx]);
        s_b[ty][tx] = float(B[(ty + i * BK) * n + ix]);
        __syncthreads();
        for (int k = 0; k < BK; k++) {
    ...
    C[iy * n + ix] = alpha * tmp + beta * C[iy * n + ix];
}
...
int main(int argc, char** argv) {
    ...
    // 初始化矩阵 A 和 B
    initialData_ez(A, m*k);
    initialData_ez(B, n*k);
    initialData_ez(C, n*m);
